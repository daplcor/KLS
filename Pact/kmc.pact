...
; ============================================
    ...
    (defschema collection-bid-schema
        @doc "stores information about the current collection bid value"
...
; ============================================
    ...
    ; (defun place-collection-bid (account:string bid:decimal)
    ;     @doc "Allows a user to place a bid on the entire collection, effectively creating a 'buyer floor'"
    ;     ;; check if new bid is greater than current bid
    ;     ;; sends coin to contract wallet
    ;     ;; returns coin to previous highest bidder's wallet
    ;     ;; update collection bid table with the new bid amount and the k:address for the bidder
    ;     ;; collect the information on how long they want the bid to be valid
    ;     ;;
    ; )

    ; (defun accept-collection-offer (account:string nft-id:string)
    ...
    ;     ;; send coins from contract wallet to the account calling the function
    ;     ;; assign the NFT to the new owner
    ...
    (defun claim-work (recipient:string nft-ids:list calculated:decimal)
        ...
        (format "Succesfully claimed {} kda from {} miners" [calculated, (length nft-ids)])
    )
...
; ============================================
    ...
    (defcap CLAIM (account:string amount:decimal)
        @doc "Emitted wheevent when a user claims KDA from the hashrate wallet"
        @event true
    )

...
(module kls-ledger GOVERNANCE
  ...
  (defcap MINT-KLS (account:string amount:decimal)
    @managed ;; one-shot for a given amount
    (compose-capability (PRIVATE))
    (compose-capability (CREDIT-KLS account))
    (compose-capability (UPDATE_SUPPLY))
  )
  ...
  (defcap CREDIT-KLS (receiver:string) true)
  ...
  (defcap CLAIM-RESERVED (account:string)
    @managed
  ...
  (defun mint-kls:bool
    ...
    (require-capability (PRIVATE))
      (let* ( (collection-info:object{collection-schema} (get-collection collection))
              ...
              (token:object{kip.token-policy-v1.token-info} (at 'token collection-info))
  ...
  (defun mint-bulk-kls
    ...
    (with-capability (MINT-KLS account amount)
    ...
    (let ( (policy:module{kip.token-policy-v1} (at 'policy (get-collection collection)))
      ...
      (map (mint-kls account guard amount collection) (make-list count 1))
    )
    )
  )
  ...
  (defun claim-reserved (account:string)
    (with-capability (CLAIM-RESERVED account)
      (let* ( (reservation-details (read-reservation account))
              (kls-reserved:integer (at 'amount-kls reservation-details))
              (guard:guard (at 'guard reservation-details)))
              (enforce-guard guard)
        (mint-bulk-kls account guard 1.0 "Kls" kls-reserved)
      )
    )
  )
  ...
  (defun credit:object{receiver-balance-change}
    ...
    (enforce-one "Capabilities not installed."
      [(require-capability (CREDIT id account))
       (require-capability (CREDIT-KLS account))])
...
(if (read-msg "initdb")
[
  (create-table free.kls-ledger.ledger)
  (create-table free.kls-ledger.tokens)
  (create-table free.kls-ledger.collections)
  ]
"No initdb, contract successfully deployed")

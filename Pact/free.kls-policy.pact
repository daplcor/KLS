...
(module kls-policy GOVERNANCE
  ...
  (implements free.kls-token-policy-v1)

  (use free.kls-token-policy-v1 [token-info])
  ...
  (defun enforce-ledger:bool ()
    (enforce-guard (free.kls-ledger.ledger-guard))
  )
  ...
  (defun enforce-bulk-mint:bool (account:string count:integer)
    ...
    (let* ( (minted:integer (get-account-minted account))
        ...
        (with-read mint-status MINT_STATUS {
         ...
         (cond
           ((and (contains account pre-sale-users) (< minted kls-reserved))
             [(enforce (<= mint-count kls-reserved)
                (format "You can Claim only {} tokens" [kls-reserved]))
                ]
           )
           (whitelist-enabled
  ...
  (defun enforce-reserved (account:string)
    (let ( (minted:integer (get-account-minted account))
           (reserved:integer (get-kls-reserved account)))
...
(if (read-msg "initdb")
[
  (create-table free.kls-policy.policies)
  (create-table free.kls-policy.collection-info)
  (create-table free.kls-policy.mint-status)
  (create-table free.kls-policy.account-details)
  (create-table free.kls-policy.whitelists)
  (initdb)
]
